...
| 环境变量前缀 `KHAOSBOX_*` | 全量改为 `CARDO_*`：含 `DEBUG_PACKAGE`、`DESKTOP_PRODUCT_NAME`、`DESKTOP_DEV_SERVER_URL`、`BUILD_PROXY`、`USE_RUNTIME` 及后续新增同前缀项 | 执行时以仓库 `rg KHAOSBOX_` / `rg __KHAOSBOX_` 全量扫描为准；设计文档已用 `CARDO_USE_RUNTIME`；禁止长期双前缀兼容 |
| Vite define `__KHAOSBOX_*` | `__CARDO_*`（如 `__CARDO_DEBUG_PACKAGE__`） | 与 env 同步改；无旧 define 双读 |
...
| Extension Worker / OPFS 内部名：Worker `khaosbox-database`；消息类型 `khaosbox-db-storage`；OPFS SAH VFS `khaosbox-opfs-sahpool` | 例如 `cardo-database` / `cardo-db-storage` / `cardo-opfs-sahpool` | 见 `extensionDatabasePort.ts`、`databaseWorker.ts`；可与 OPFS 写硬禁用（PR5）同 PR，或随删死代码一并处理；禁止长期双名 shim |
...
| UI 封装目录 `src/web-next/ui/khaos` 与组件前缀 | 可随视觉/UI rename 另 PR 再改为 Cardo 系命名 | 不阻塞 Runtime 功能 PR；终态 Agents 改写（PR6）时再对齐目录与文案 |
...
AGENTS.md 禁止旧 schema/字段双读 shim；「改路径导致用户数据消失」是数据丢失，不是双读，必须显式策略。
...
- [x] 本改名清单可执行
- [x] `AGENTS.md` 含过渡注记，指向设计文档与目标架构，禁止把 OPFS 权威写 / Desktop raw SQL IPC 当长期设计

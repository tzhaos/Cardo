...
import { OperationJournalPanel } from './OperationJournalPanel';

export function OperationJournalWindow() {
  const menu = useIndependentMenuStore((state) => state.menus.journal);
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') closeMenu('journal');
    };
  ...
  const beginDrag = (event: ReactPointerEvent<HTMLElement>) => {
    if ((event.target as HTMLElement).closest('button,input,textarea,[data-no-menu-drag]')) return;
    const element = windowRef.current;
    if (!element) return;
    ...
    const session = startWindowPointerSession({
      ...
      onEnd: () => {
        ...
        moveMenu('journal', latestPosition);
      },
    });
  ...
  return (
    <AnimatePresence>
      {menu.open ? (
        <motion.aside
          className="wbn-independent-menu wbn-journal-window"
          data-independent-menu="journal"
          id="wbn-journal-window"
          ref={windowRef}

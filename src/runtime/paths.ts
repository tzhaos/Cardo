 ...
 *
 * v1 keeps the Desktop SQLite path so attach/embed share one DB (design §2.1).
 * Desktop today: path.join(app.getPath('userData'), 'khaosbox.sqlite').
 * Electron userData for package name "khaosbox" is typically:
 *   win32:  %APPDATA%/khaosbox
 *   darwin: ~/Library/Application Support/khaosbox
 *   linux:  ~/.config/khaosbox
 *
 * Override with CARDO_DATA_DIR (absolute directory). This module is the shared
 * SoT with Desktop PR4 — do not invent a second default under Cardo/cardo.sqlite.
 */
...
/** Electron / product package name used for userData directory (not display productName). */
 ...
 * Matches Electron app.getPath('userData') for name=khaosbox when CARDO_DATA_DIR is unset.
 */

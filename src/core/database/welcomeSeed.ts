...
const WELCOME_COPY: Record<PreferenceLocale, WelcomeSeedCopy> = {
  ...
  en: {
    boxTitle: 'Hello',
    tips: [
      'Welcome to Cardo. Use boxes to collect things, and groups to organize different workspaces.',
      ...
      'Use the bottom button to create a box. The sidebar has search, new groups, Favorites, and the Recycle Bin.',
...
/** First-run default group titles (Workspaces / Personal / Inspiration). */
export function getDefaultGroupTitles(
  locale: PreferenceLocale,
): readonly [workspaces: string, personal: string, inspiration: string] {
  if (locale === 'zh') return ['工作', '个人', '灵感'];
  return ['Workspaces', 'Personal', 'Inspiration'];
}

...
- Native Host：用于打开本地文件和文件夹的 TypeScript Native Messaging Host。
...
|-- web/         # React UI、Zustand store、用例和功能控制层
...
浏览器扩展和 Electron 桌面端复用 `src/web`。平台能力通过 `src/core/ports` 注入。

## 本地资源

文件和文件夹通过可选的 Native Messaging Host 打开。扩展向 `com.khaosbox.local_bridge` 发送 `open-local-resource` 消息，host 再调用系统文件管理器打开本地路径。Electron 桌面端通过自己的 preload IPC bridge 打开本地资源。

...
> [!NOTE]
> 文件和文件夹的打开仍然依赖系统中已经注册好的 `kbe:` 协议处理器。当前仓库中已经包含位于 `native-host/windows/build/win-x64/` 的 Windows native-host 构建产物，但处理器的安装与注册仍然在扩展之外完成，而且仓库里暂时没有提供对应的安装说明。
